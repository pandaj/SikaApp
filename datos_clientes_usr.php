...
foreach ($clientes as $cliente) :
	$useroID = $cliente['user_id'];
	$usersNames[ $useroID ]['clientes'][] = array(
		$cliente['cliente_id'],
	...
	<div class="app-body">
		...
		<main class="main">
			...
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active">Clientes Por Usuario</li>
			</ol>
								...
								<div class="card-body" id="tabla-datos">
									...
									<ul class="nav nav-tabs" role="tablist">
										...
										foreach ($usersNames as $usero) :
											if (count($usero['clientes']) > 1) :
												$n++;
												$activo = '';
												if ($n == 1) $activo = ' active';
												$elName = 'user'.$usero['user_id'];
												?>
											...
											if (count($usero['clientes']) > 1) :
												...
												<div class="tab-pane<?php echo $activo; ?>" id="<?php echo $elName; ?>" role="tabpanel">
                                                	<div class="card-header">
														...
														<b><?php echo $usero['nombre']; ?></b>
													</div>
													<form class="card-body" style="padding-bottom:0;" method="post" action="adminExcel.php">
														<input type="hidden" name="datos" value='<?php echo json_encode($usero['clientes']); ?>' />
													...
													<table class="table table-responsive-sm table-striped">
														<?php
														$n = 0;
														foreach ($usero['clientes'] as $cliente) :
															$n++;
															if ($n == 1) :
																?>
																<thead><tr>
